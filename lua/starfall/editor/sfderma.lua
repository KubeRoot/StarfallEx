...
-- Starfall Per-Player Permissions
PANEL = {}
...
local function createpermissionsPanel (parent)
	...
	for id, setting in SortedPairs( clientProvider.settings ) do
		if not chip.instance.permissionsOverrideRequests[id] then
			continue
		end
		local descriptions = clientProvider.settingsdesc[id]
		...
		settingtext:SetText( string.format("%s - %s",id,descriptions[1]) )
...
function PANEL:OpenForChip(chip)
	self.chip = chip
	self.description:SetText(chip.instance.permissionsDescription)
